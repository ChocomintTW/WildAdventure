...
public class ModHud extends DrawableHelper {
	...
	public void render(MatrixStack matrices) {
		...
		if (player != null && client.interactionManager != null
			...
			IGameOption options = ((IGameOption) MinecraftClient.getInstance().options);
			WaterRenderType renderType = options.getWaterRenderType().getValue();
			if (renderType == WaterRenderType.ALWAYS_RENDER || (renderType == WaterRenderType.KEY_PRESSED && InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), KeyBindingHelper.getBoundKeyOf(ModKeyBindings.RENDER_WATER).getCode()))
					&& player.getOffHandStack().isEmpty())
			{
				...
				TemperatureScale scale = options.getTemperatureScale().getValue();
